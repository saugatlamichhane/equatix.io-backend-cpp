...
void Profile::getInfo(
  ...
  drogon::async_run([db, uid, cb]() -> drogon::Task<> {
    try {
      ...
      result["bestElo"] = row["best_elo"].isNull() ? row["elo"].as<double>()
                                                   : row["best_elo"].as<int>();
      result["bestWinStreak"] = row["best_win_streak"].isNull()
                                    ? 0
                                    : row["best_win_streak"].as<int>();
      result["currentStreak"] = row["current_win_streak"].isNull()
                                    ? 0
                                    : row["current_win_streak"].as<int>();
      ...
      result["totalPlayTimeHours"] =
          static_cast<int>(round((totalGameSeconds / 3600.0) * 10)) / 10.0;
