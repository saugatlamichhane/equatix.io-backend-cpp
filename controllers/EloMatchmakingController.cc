...
void EloMatchmakingController::handleNewMessage(
  ...
  drogon::async_run(
      [db, wsConnPtr, opponentWsConn, userId, opponentId]() -> drogon::Task<> {
        try {
          ...
          opponentWsConn->send(
              Json::writeString(Json::StreamWriterBuilder(), resp));
...
void EloMatchmakingController::handleConnectionClosed(
...
void EloMatchmakingController::handleNewConnection(
    const HttpRequestPtr &, const WebSocketConnectionPtr &) {
  // Nothing needed
