#include "LiveStatsController.h"
#include <atomic>
#include <chrono>
#include <ctime>
#include <drogon/utils/coroutine.h>
#include <iomanip>
#include <memory>
#include <sstream>
#include <trantor/utils/Logger.h>

using namespace drogon::orm;

// Initialize static members
std::atomic<size_t> LiveStatsController::activePlayersCount_{0};

void LiveStatsController::incrementActivePlayers() { activePlayersCount_++; }

void LiveStatsController::decrementActivePlayers() {
  if (activePlayersCount_ > 0) {
    activePlayersCount_--;
  }
}

size_t LiveStatsController::getActivePlayersCount() {
  return activePlayersCount_;
}

void LiveStatsController::getLiveStats(
    const HttpRequestPtr &req,
    std::function<void(const HttpResponsePtr &)> &&callback) {
  auto db = drogon::app().getDbClient();
  auto cb = std::make_shared<std::function<void(const HttpResponsePtr &)>>(
      std::move(callback));

  drogon::async_run([db, req, cb]() -> drogon::Task<> {
    try {
      auto resp = HttpResponse::newHttpResponse();
      resp->setContentTypeCode(CT_APPLICATION_JSON);
      Json::Value result;

      auto uid = req->attributes()->get<std::string>("uid");
      if (uid.empty()) {
        resp->setStatusCode(k401Unauthorized);
        result["error"] = "Unauthorized - no user ID found";
        resp->setBody(Json::writeString(Json::StreamWriterBuilder(), result));
        (*cb)(resp);
        co_return;
      }

      size_t playersOnline = getActivePlayersCount();
      result["players_online"] = static_cast<Json::UInt>(playersOnline);

      auto gamesTodayResult = co_await db->execSqlCoro(
          "SELECT COUNT(*) as game_count FROM challenges WHERE status = "
          "'completed' AND DATE(created_at) = CURRENT_DATE");
      int gamesToday = gamesTodayResult[0]["game_count"].as<int>();
      result["games_today"] = gamesToday;

      auto userResult =
          co_await db->execSqlCoro("SELECT elo FROM users WHERE uid = $1", uid);
      if (userResult.empty()) {
        resp->setStatusCode(k404NotFound);
        result["error"] = "User not found";
        resp->setBody(Json::writeString(Json::StreamWriterBuilder(), result));
        (*cb)(resp);
        co_return;
      }
      double userElo = userResult[0]["elo"].as<double>();
      result["your_rating"] = userElo;

      auto rankResult = co_await db->execSqlCoro(
          "SELECT COUNT(*) + 1 as rank FROM users u1 WHERE u1.elo > (SELECT "
          "elo FROM users WHERE uid = $1)",
          uid);
      int userRank = rankResult[0]["rank"].as<int>();
      result["your_rank"] = userRank;

      resp->setStatusCode(k200OK);
      resp->setBody(Json::writeString(Json::StreamWriterBuilder(), result));
      (*cb)(resp);
    } catch (const DrogonDbException &e) {
      LOG_ERROR << "Database error in getLiveStats: " << e.base().what();
      auto resp = HttpResponse::newHttpResponse();
      resp->setContentTypeCode(CT_APPLICATION_JSON);
      resp->setStatusCode(k500InternalServerError);
      Json::Value result;
      result["error"] = "Database error";
      resp->setBody(Json::writeString(Json::StreamWriterBuilder(), result));
      (*cb)(resp);
    } catch (const std::exception &e) {
      LOG_ERROR << "Error in getLiveStats: " << e.what();
      auto resp = HttpResponse::newHttpResponse();
      resp->setContentTypeCode(CT_APPLICATION_JSON);
      resp->setStatusCode(k500InternalServerError);
      Json::Value result;
      result["error"] = "Internal server error";
      resp->setBody(Json::writeString(Json::StreamWriterBuilder(), result));
      (*cb)(resp);
    }
    co_return;
  });
}