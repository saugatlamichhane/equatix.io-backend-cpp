...
void FirebaseAuthFilter::refreshKeys() { fetchFirebaseKeys(); }
...
void FirebaseAuthFilter::doFilter(const HttpRequestPtr &req,
  ...
  try {
    ...
    if (authHeader.size() < kBearer.size() ||
        std::string_view(authHeader).substr(0, kBearer.size()) != kBearer) {
      throw std::runtime_error("Invalid format");
    }
    std::string token = authHeader.substr(kBearer.size());
